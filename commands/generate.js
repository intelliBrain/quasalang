...
module.exports = function () {
  this.generate = function (options) {
    const watermark = "This file was auto-generated by quasalang-ib";
    let watching = false;
      ...
      fs.createReadStream(options.input)
        ...
        .on("end", () => {
          let languagesAndCodesAsObjects =
            getLanguagesAndCodesAsObjects(results);
          ...
          let maxIndex = languagesAndCodesAsObjects.length - 1;
          languagesAndCodesAsObjects.forEach((langObj, index) => {
            ...
            mainIndexFile += `  ${codeText}: ${langObj.codeAsVariable}${lineComma} // ${langObj.lang}\n`;
          });
          mainIndexFile += `}\n`;
            ...
            if (!options.force) {
              prompt([
                ...
                if (answers.confirmDeleteOutputFolder) {
                  console.log(
                    "INFO: Skip this prompt in future with the --force (or -f) option."
                  );
                  deleteOutputFolder();
                }
              });
            ...
            fs.writeFile(
              ...
              function (err) {
                ...
                filesWrittenMessage.push({
                  ...
                  Code: "",
                  Path: `${options.output}/index.ts`,
                });

                // generate individual language folders and index.ts files
                ...
                languagesAndCodesAsObjects.forEach((langObj) => {
                  ...
                  results.forEach((result, index) => {
                    ...
                    if (result.Key) {
                      ...
                      if (result.Key.startsWith("#")) {
                        languageIndexFile += `  // ${result.Key.substring(
                          1
                        ).trim()}\n`;
                      }
                  ...
                  let languageIndexFilePath = `${options.output}/${langObj.code}/index.ts`;
                  fs.writeFile(
                    `${languageIndexFilePath}`,
                    languageIndexFile,
                    function (err) {
                      if (err) {
                        return console.log(err);
                      ...
                      if (
                        languageFilesWritten ===
                        languagesAndCodesAsObjects.length
                      ) {
                        logWriteMessages();
                      }
                    }
